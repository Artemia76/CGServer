...
// *   Le 22/04/2005                                                           *
// *   Modifie 12/07/2008                                                      *
// *   CopyRight (c)Neophile 2005-2008                                         *
// *                                                                           *
// *****************************************************************************
// *                                                                           *
...
CDBase* CDBase::Create ()
{
	if (!CDBase::PtCDBase) CDBase::PtCDBase = new CDBase ();
	return CDBase::PtCDBase;
}
...
CDBase::CDBase()
{
	...
	for (int i=0 ; i<nb_user ; i++)
	{
		...
		pConfig->Read(s+_T("Nom"), &Users[i].Name, _T("Anonyme"));
		...
		pConfig->Read(s+_T("Titre/Texte"), &Users[i].TexteTi, "");
		pConfig->Read(s+_T("Qualite/Enabled"), &Users[i].Quali, 0);
		pConfig->Read(s+_T("Qualite/Texte"), &Users[i].TexteQu, "");
...
void CDBase::Sauve (int id)
{
	...
	pConfig->Write(s+_T("Nom"), Users[id].Name);
	pConfig->Write(s+_T("Citoyen"), (long)(Users[id].Citizen));
...
int CDBase::Identifie (wxString& Nom, unsigned int Citoyen)
{
	...
	for (int i=0 ; i< nb_user ; i++)
	{
		if (Users[i].Name.Upper()==Nom.Upper())
		{
	...
	{
	// Dernière solution, on cherche la plus petite fréquence pour remplacer un vieil
...
CColour CDBase::GetColor (int id)
{
	CColour noir(0,0,0);
...
void CDBase::SetColor (int id, CColour& Couleur)
{
...
{
	if (id<nb_user) return Users[id].Name;
	else
		return _T("");
}
...
int CDBase::Search (wxString Nom)
{
	...
	for (int i=0; i < nb_user ; i++)
	{
		if (Users[i].Name.Upper()==Nom) return i;
	}
	return -1;
}
...
wxArrayString CDBase::GetMuteList ()
{
	...
	for (int i=0; i < nb_user ; i++)
	{
		if (Users[i].Mute) Resultat.Add(Users[i].Name);
	}
	return Resultat;
}
...
bool CDBase::GetModo (int id)
{
	return Users[id].Modo;
}

void CDBase::SetModo (int id, bool Modo)
{
	Users[id].Modo=Modo;
	Sauve(id);
}
